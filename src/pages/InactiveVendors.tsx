...
export default function InactiveVendors() {
  ...
  const [sendingLocationToAll, setSendingLocationToAll] = useState(false);
  ...
  const handleSendLocationUpdateToAll = async () => {
    const confirmed = window.confirm(
      `Are you sure you want to send location update messages to ALL vendors?\n\nThis will send the location update template to every vendor in the system.\n\nThis action cannot be undone.`
    );
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('Bulk location update send result:', result);
        
        // Show results
        if (result.errorCount === 0) {
          alert(`✅ Successfully sent location update messages to all ${result.sentCount} vendors!`);
        } else {
      ...
      console.error('Error sending location updates to all vendors:', err);
      alert('Error sending location updates to all vendors. Please try again.');
    } finally {
      setSendingLocationToAll(false);
    }
  };
    ...
    <AdminLayout>
        ...
        <div className="mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 lg:gap-4">
              <button
                onClick={handleSendLocationUpdateToAll}
                disabled={sendingLocationToAll}
                className="px-3 py-2 lg:px-4 lg:py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-green-400 disabled:cursor-not-allowed transition-colors text-sm lg:text-base flex items-center gap-2"
              >
                {sendingLocationToAll ? (
                  <>
